...
- **What could be improved** — Review the implementation with a product eye. Identify areas where the user experience, code quality, performance, or test coverage could be better. Be specific and actionable (e.g., "The error state in StationListView shows a generic message — could show network vs. server errors distinctly" or "The search debounce is 300ms but feels sluggish with large lists — consider 150ms"). Focus on improvements that would meaningfully impact the user or developer experience, not nitpicks.
- **Best next step** — Based on SPEC.md and PLAN.md, recommend the single most valuable next `/implement` command the user should run. Consider: what unblocked features depend on what was just built? What would deliver the most user-facing value? What is the natural continuation of the current work? Format as a concrete suggestion: *"Run `/implement <feature>` — it builds on the service layer just added and would complete the core listening experience."*
