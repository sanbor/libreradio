...
- `AudioPlayerService` uses `@MainActor` (not `actor`) because `AVPlayer` requires main-thread access. Uses KVO on `player.timeControlStatus` and `playerItem.status` for state transitions. ICY stream metadata is captured via `AVPlayerItemMetadataOutput` delegate (not the deprecated `timedMetadata` KVO). The delegate is a separate `NSObject` subclass (`MetadataOutputHandler`) because `AVPlayerItemMetadataOutputPushDelegate` requires `NSObjectProtocol` conformance, which `AudioPlayerService` doesn't have. Uses `await item.load(.stringValue)` for non-deprecated metadata access.
- **ICY metadata identifier trap (2026-04-10 regression fix):** When the old `timedMetadata` KVO was replaced with `AVPlayerItemMetadataOutput` (commit `aa62ee9`), the filter was translated from `metadata.commonKey == AVMetadataKey.commonKeyTitle` to `item.identifier == .commonIdentifierTitle`. These are *not* equivalent: `commonKey` is the *abstract common-mapped* key, whereas `item.identifier` returns the *native* metadata identifier. ICY/Shoutcast/Icecast streams deliver their stream title with `AVMetadataIdentifier.icyMetadataStreamTitle` — filtering only on `.commonIdentifierTitle` silently dropped every ICY title, which is what the vast majority of Radio Browser stations emit. The fix: filter against the set `{ .commonIdentifierTitle, .icyMetadataStreamTitle }`. Regression tests in `AudioPlayerServiceTests` assert both identifiers are accepted and drive a synthetic `AVMutableMetadataItem` with `.icyMetadataStreamTitle` through the delegate to verify end-to-end parsing.
- **Dropped-identifier debug log:** To make future unrecognized-identifier cases easier to spot before they become bugs, `MetadataOutputHandler` logs each dropped item whose identifier is non-nil but not in `titleIdentifiers` via `os.Logger` at `.debug` level (subsystem `com.libreradio`, category `MetadataOutputHandler`). Items with `nil` identifiers are not logged — those are an AVFoundation no-op case, not "unrecognized." The log closure is injectable via `MetadataOutputHandler(logUnrecognizedIdentifier:)` so tests can capture identifiers without reading from the unified logging system; the default binds to a `static let logger` to avoid allocating a `Logger` per dropped item.
...
- **Landscape spacer tuning:** The top spacer behavior differs between flat and alphabetical lists. Flat lists (sorted by clicks) need NO top spacer — `.searchable(.navigationBarDrawer)` provides sufficient automatic insets. Alphabetical lists need a 28pt spacer so the first alphabet index letter is visible below the search bar. The original 56pt was too large; 16pt was too small for the index but too large for the flat list.
- **Landscape bottom padding:** The portrait `listBottomPadding` (80pt) is insufficient in landscape because the compact tab bar is shorter than the hardcoded `tabBarHeight = 49pt` used to position the mini player overlay. Landscape lists use 160pt bottom spacer to ensure the last item clears the floating mini player.
...
**Implementation notes (Instant Startup — Cache Optimization):**
- **Batch cache loading eliminates actor hop overhead:** `HomeViewModel.load()` previously made 5 sequential `await cache.load()` calls, each requiring an actor hop to `StationCacheService` and back to `@MainActor`. Replaced with a single `loadHomeData(localCountryCode:)` method that reads all 5 keys in one actor entry. The `HomeCacheData` struct is `Sendable` so it crosses the actor boundary efficiently.
- **`isLoading = false` immediately after cache assignment:** The loading spinner (`isLoading && topByClicks.isEmpty`) flashed briefly even with warm cache because `isLoading` stayed `true` during all cache reads. Now `isLoading` is cleared right after cached data is assigned to `@Published` properties, before the network fetch begins. The network fetch runs silently — cached data is already visible.
- **Favicon memory pre-warming on startup:** `ImageCacheService.preWarmMemoryCache(for:)` reads disk-cached favicons into `NSCache` for up to 60 URLs. Called as a background `Task` after cache assignment in `HomeViewModel.load()`. This eliminates the placeholder-to-image flash in `FaviconImageView`, whose synchronous `cachedImage(for:)` check now hits warm memory instead of falling through to async disk I/O.
- **`preWarmMemoryCache` is bounded:** Caps at 60 URLs via `.prefix(60)` to limit worst-case work (~60 SHA256 hashes + disk reads). Covers all above-the-fold sections (favorites, recents, local, top clicks, top votes). Below-the-fold sections (recently changed, currently playing) load lazily via `FaviconImageView`'s existing async path.
